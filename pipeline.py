...
def _load_macrobond():
    """Lazy-load the Macrobond specialist — only available when license is active."""
    try:
        from newsletter_agent.specialists.macrobond import fetch_macrobond
        return fetch_macrobond
    except Exception:
        def _unavailable(task):
            print("  [macrobond] Specialist not available — set MACROBOND_CLIENT_ID/SECRET in .env")
            return {"dataframes": {}, "kilde": [], "chart_specs": task.get("charts", [])}
        return _unavailable
...
def _build_before_after_bars(dfs: dict, chart_spec: dict, kilde_str: str, output_path: str) -> str:
    ...
    if len(snapshot_df) > 1:
        ...
        if abs_vals and max(abs_vals) / (min(abs_vals) + 1e-9) > 10:
            ...
            # render_type_b handles single-column with pos/neg colouring
            pct_changes = {
                lbl: (after_vals[lbl] - before_vals[lbl]) / abs(before_vals[lbl]) * 100
                for lbl in before_vals
            }
            pct_df = pd.DataFrame({"% endring": pct_changes})
            return render_type_b(pct_df, {**chart_spec, "y_label": "% ændring",
                                          "kilde": kilde_str}, output_path)
...
def run(brief: str, output_dir: str = "output",
        start_date: str = "", end_date: str = "", period_days=None) -> list:
    """
    Main pipeline entry point.
    brief: free-form topic string from department.
    output_dir: where to save PNG files and manifest.json.
    start_date / end_date: ISO strings ("YYYY-MM-DD") — hard clip applied after fetch.
    period_days: int override for all series windows (ignored when start_date/end_date set).
    Returns list of FigurePackage dicts: [{"path": str, "metadata": dict}, ...]
    """
    os.makedirs(output_dir, exist_ok=True)

    # Normalise date override params
    _start_ts = pd.Timestamp(start_date) if start_date else None
    _end_ts   = pd.Timestamp(end_date)   if end_date   else None
    _period_days = int(period_days) if period_days is not None else None
    ...
    # Hard-clip all fetched DataFrames to user-specified date window
    if _start_ts is not None or _end_ts is not None:
        for sp_name, result in specialist_results.items():
            clipped = {}
            for label, df in result["dataframes"].items():
                if df is None or df.empty:
                    clipped[label] = df
                    continue
                mask = pd.Series(True, index=df.index)
                if _start_ts is not None:
                    mask = mask & (df.index >= _start_ts)
                if _end_ts is not None:
                    mask = mask & (df.index <= _end_ts)
                clipped[label] = df.loc[mask]
            result["dataframes"] = clipped
        clip_desc = f"{start_date} → {end_date}"
        print(f"  [pipeline] Date window enforced: {clip_desc}")
